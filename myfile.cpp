...
if (b == 0) 
{
v[0] = 1; v[1] = 0; v[2] = a; return ;
}
 extendgcd(b, a % b, v); int x = v[1]; v[1] = v[0] - v[1] * (a / b); v[0] = x; return;
}
 //pass an arry of size1 3
int mminv(int a, int b) 
{
int arr[3]; extendgcd(a, b, arr); return arr[0];
}
 //for non prime b
...
{
int val1 = fact[n]; int val2 = ifact[n - r]; int val3 = ifact[r]; return (((val1 * val2) % m) * val3) % m;
}
...
{
  vector<int> arr(n + 1, true);  vector<int> vect; for (int i = 2; i <= n; i++)   if (arr[i] == 1)   
{
      vect.push_back(i);      for (int j = 2 * i; j <= n; j += i)           arr[j] = 0;    
}
 return arr;
}
